...
# python finetune_on_qm9_seq.py --pretrain_models /share/project/sharefs-skfeng/pre-training-via-denoising/experiments/pretrain_models/ET-PCQM4MV2_dih_var0.04_var2_com_re/step=399999-epoch=8-val_loss=0.1422-test_loss=0.2784-train_per_step=0.1512.ckpt

# python finetune_on_qm9_seq.py --pretrain_models /share/project/sharefs-skfeng/pre-training-via-denoising/experiments/pretrain_models/ET-PCQM4MV2_dih_var0.04_var2_com_re/step=399999-epoch=8-val_loss=0.1422-test_loss=0.2784-train_per_step=0.1512.ckpt --seeds 3 --conf ET-QM9-FT-nt_dw_0.2_long.yaml
...
# qm9_task = {'energy_U0': 7, 'enthalpy_H': 9, 'free_energy': 10}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seeds", type=int, default=0) # seed number,default 0
        ...
        for seed in range(1, seeds + 1):
            for i, task in enumerate(qm9_task):
                ...
                # base_cmd = f'srun -mem=64000 --gres=gpu:1 --time 6-12:00:00 --job-name {job_prefix}_qm9_{task}_finetuning python -u scripts/train.py  scripts/train.py --conf examples/ET-QM9-FT.yaml --layernorm-on-vec whitened --job-id {job_prefix}_qm9_{task}_finetuning --dataset-arg {task} --pretrained-model {pretrain_model} > {job_prefix}_qm9_{task}_finetuning.log 2>&1 &'
                    base_cmd = f'CUDA_VISIBLE_DEVICES={i} python -u scripts/train.py --conf examples/{config} --layernorm-on-vec whitened --job-id {job_prefix}_qm9_{task}_{cmd_add}finetuning_seed{seed} --dataset-arg {task} --pretrained-model {pretrain_model} {addition_cmd} --seed {seed} > {job_prefix}_qm9_{task}_{cmd_add}finetuning_seed{seed}.log 2>&1 &'
                # if i < (task_len - 1):
                #     base_cmd += ' 2>&1 &'
                print(base_cmd)
                # os.system(base_cmd)
