...
def createConformations(outputfile, forcefield, smiles, sid):
    ...
    def simulate(pos):
        integrator = openmm.LangevinMiddleIntegrator(500*openmm.unit.kelvin, 1/openmm.unit.picosecond, 0.001*openmm.unit.picosecond)
        simulation = app.Simulation(mmtop, system, integrator, openmm.Platform.getPlatformByName('Reference'))
        simulation.context.setPositions(pos.m_as('angstrom'))
        simulation.minimizeEnergy()
        simulation.context.setVelocitiesToTemperature(500*openmm.unit.kelvin)
        states = []
        for i in range(10):
            simulation.step(10000)
            state = simulation.context.getState(getPositions=True, getEnergy=True)
            if state.getPotentialEnergy() < 1e4*openmm.unit.kilojoules_per_mole:
                states.append(state)
        return states
    ...
    integrator = openmm.LangevinMiddleIntegrator(100*openmm.unit.kelvin, 1/openmm.unit.picosecond, 0.001*openmm.unit.picosecond)
    simulation = app.Simulation(mmtop, system, integrator, openmm.Platform.getPlatformByName('Reference'))
    for state in states[:]:
...
index = 0
for line in open('simulate/sorted.txt'):
    index += 1
    if index < first:
        continue
    if index > last:
